...
-- Q.43 Show all of the patients grouped into weight groups.
-- Show the total amount of patients in each weight group. Order the list by the weight group decending.
-- For example, if they weight 100 to 109 they are placed in the 100 weight group, 110-119 = 110 weight group, etc.
-- Soln
SELECT 
    COUNT(*) AS amt_of_patients,
    FLOOR(weight / 10) * 10 AS weight_group
FROM patients
GROUP BY weight_group
ORDER BY weight_group DESC;

-- Q.44 Show patient_id, weight, height, isObese from the patients table. Display isObese as a boolean 0 or 1.
...
-- Q.45 Show patient_id, first_name, last_name, and attending doctor's specialty.
...
-- Q.46 All patients who have gone through admissions, can see their medical documents on our site. 
...
SELECT DISTINCT p.patient_id,
	CONCAT(p.patient_id,
		   LEN(p.last_name),
		   YEAR(birth_date)) AS temp_pass
FROM patients p
JOIN admissions a ON p.patient_id = a.patient_id;

-- Q. 47 Each admission costs $50 for patients without insurance, and $10 for patients with insurance.
...
-- Q.48 Show the provinces that has more patients identified as 'M' than 'F'. Must only show full province_name
-- Soln
...
-- Q.49 We are looking for a specific patient. Pull all columns for the patient who matches the following criteria:
...
-- Q.50 Show the percent of patients that have 'M' as their gender. Round the answer to the nearest hundreth number and in percent form.
-- output: 54.48%
-- Soln
SELECT CONCAT(ROUND(((SELECT COUNT(*) FROM patients WHERE gender = 'M')*100)/CAST(COUNT(*) AS float),2),'%') AS percent_of_male_patients FROM patients;

-- Q.51 For each day display the total amount of admissions on that day. Display the amount changed from the previous date
-- Soln
...
-- Q.52 Sort the province names in ascending order in such a way that the province 'Ontario' is always on top.
-- Soln
SELECT province_name
FROM province_names
ORDER BY (CASE
...
-- Q.53  We need a breakdown for the total amount of admissions each doctor has started each year.
